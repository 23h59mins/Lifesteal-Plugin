...
public class AdminCommands implements CommandExecutor {
    ...
    private void handleGiveHeart(CommandSender sender, String[] args) {
        ...
        String msg = plugin.getLangManager().get("admin.giveheart-success")
                .replace("{player}", target.getName())
                .replace("{tier}", String.valueOf(tier))
                .replace("{amount}", String.valueOf(amount));
        MessageUtils.sendWithPrefix(sender, plugin, msg);
    }
    ...
    private void handleReviveBeacon(CommandSender sender, String[] args) {
        ...
        String msg = plugin.getLangManager().get("admin.revivebeacon-given")
                .replace("{player}", target.getName());
        MessageUtils.sendWithPrefix(sender, plugin, msg);
    }
    ...
    private void handleSet(CommandSender sender, String[] args) {
        ...
        Player target = parseTarget(sender, args, 1);
        Integer amount = parseAmountOrNull(sender, args[2]); if (target == null || amount == null) return;
    ...
    private void handleResetAll(CommandSender sender) {
        ...
        Bukkit.getOnlinePlayers().forEach(p -> plugin.getHeartManager().setHearts(p, startingHearts));
    ...
    private int parseAmount(CommandSender sender, String[] args, int argIndex, int defaultValue) {
        try {
            if (args.length > argIndex) {
                return Math.max(Integer.parseInt(args[argIndex]), 1);
            }
